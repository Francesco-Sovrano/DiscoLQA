...
def get_scores(x):
	with open(os.path.join(path,f'{x}.json'),'r') as f:
		x_results = json.load(f)["total_scores"]
	x_results_keys, x_results_values = zip(*sorted(x_results.items(), key=lambda x:x[0]))
	p_list = list(map(lambda y: y["precision"], x_results_values))
	f1_list = list(map(lambda y: y["f1"], x_results_values))
	ndcg_list = list(map(lambda y: y["ndcg"], x_results_values))
	rr_list = list(map(lambda y: y["rr"], x_results_values))
	return {
		'question': x_results_keys,
		'precision': p_list, 
		'f1': f1_list, 
		'ndcg': ndcg_list,
		'rr': rr_list,
...
def test_hypothesis(a, b):
	...
	for alternative in alternatives:
		m = scipy_stats.wilcoxon(a_value, b_value, alternative=alternative)
		stats_test_dict[b_label + ' is ' + alternative] = {
			'statistic': m.statistic,
			'pvalue': m.pvalue,
		}
	k = scipy_stats.kruskal(a_value,b_value) # Due to the assumption that H has a chi square distribution, the number of samples in each group must not be too small. A typical rule is that each sample must have at least 5 measurements.
	return {
...
for a in alternatives:
	alternative_scores = get_scores(a)
	print('Precision:', json.dumps(test_hypothesis((baseline_scores['precision'],baseline),(alternative_scores['precision'],a)), indent=4))
	print('F1:', json.dumps(test_hypothesis((baseline_scores['f1'],baseline),(alternative_scores['f1'],a)), indent=4))
	print('NDCG:', json.dumps(test_hypothesis((baseline_scores['ndcg'],baseline),(alternative_scores['ndcg'],a)), indent=4))
	print('MRR:', json.dumps(test_hypothesis((baseline_scores['rr'],baseline),(alternative_scores['rr'],a)), indent=4))
	print('Percentage of less precise answers:', r3(100*sum([
		1
		for x,y in zip(baseline_scores['precision'],alternative_scores['precision'])	
		if x > y
	])/len(baseline_scores['precision'])))
	print('Percentage of more precise answers:', r3(100*sum([
		1
		for x,y in zip(baseline_scores['precision'],alternative_scores['precision'])	
		if x < y
	])/len(baseline_scores['precision'])))
	print('Percentage of more/less precise answers:')
	for s in ['L','N','H']:
		print('#'*10)
		print('Specificity', s)
		s_baseline_scores = {
			k: [
				v
				for q, v in zip(baseline_scores['question'], baseline_scores[k])
				if question_dict[q]['specificity']==s
			]
			for k in ['precision','f1','ndcg','rr']
		}
		s_alternative_scores = {
			k: [
				v
				for q, v in zip(alternative_scores['question'], alternative_scores[k])
				if question_dict[q]['specificity']==s
			]
			for k in ['precision','f1','ndcg','rr']
		}
		w = r3(100*sum([
			1 
			for x,y in zip(s_baseline_scores['precision'],s_alternative_scores['precision']) 
			if x > y 
		])/len(s_baseline_scores['precision']))
		b = r3(100*sum([
			1 
			for x,y in zip(s_baseline_scores['precision'],s_alternative_scores['precision']) 
			if x < y 
		...
		print(f'Better: {b}%')
		print(f'Worse: {w}%')
		print('Precision:', json.dumps(test_hypothesis((s_baseline_scores['precision'],baseline),(s_alternative_scores['precision'],a)), indent=4))
		print('F1:', json.dumps(test_hypothesis((s_baseline_scores['f1'],baseline),(s_alternative_scores['f1'],a)), indent=4))
		print('NDCG:', json.dumps(test_hypothesis((s_baseline_scores['ndcg'],baseline),(s_alternative_scores['ndcg'],a)), indent=4))
		print('MRR:', json.dumps(test_hypothesis((s_baseline_scores['rr'],baseline),(s_alternative_scores['rr'],a)), indent=4))
	print('')
