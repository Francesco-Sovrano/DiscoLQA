...
//------------ YAI4Law ------------
	...
	// },
	{
		'keys': ['my:content', 'my:docID', 'my:hasIDX', 'my:article_id', 'my:paragraph_id', 'my:block_id', 'my:section_id', 'my:chapter_id', 'my:recital_id', 'my:reference_id'],
		'optional_keys': ['my:hasIDX', 'my:article_id', 'my:paragraph_id', 'my:block_id', 'my:section_id', 'my:chapter_id', 'my:recital_id', 'my:reference_id'],
		'hide_descendants': true,
		'template_fn': (is_in_array, ancestor_predicate, value_list) => {
			var [content,doc_url,label_idx,article_id,paragraph_id,block_id,section_id,chapter_id,recital_id,reference_id] = value_list;
			var head_list = [];
			var value_list = [];
			// Get article
			var article = [];
	...
	{ 
		...
		'template_fn': (is_in_array, ancestor_predicate, value_list) => {
			...
			if (source)
			{
				...
				var source_details = source;
				if (p('my:paraID') in source) {
					source_details = source[p('my:paraID')];
				...
				var recital = [];
				if (recital_id)
					recital.push(get_description(recital_id));
				// Get article
				var article = [];
				if (article_id)
					article.push(get_description(article_id));
				...
				if (recital.length > 0)
				{
					head_list.push('Recital');
					value_list.push(recital.map(x=>linkify(x,get_known_label(x))).join('.'));
				}
				if (article.length > 0)
				{
